...
export const getStaticProps = async (context: GetStaticPropsContext) => {
  ...
  const response = await axios.get(
    `https://restcountries.eu/rest/v2/alpha/${slug}?fields=name;nativeName;region;subregion;capital;population;topLevelDomain;currencies;languages;borders;alpha3Code;flag`,
  )
  ...
  const borders =
    borderCountries &&
    (await axios.get(
      `https://restcountries.eu/rest/v2/alpha?fields=name;alpha3Code&codes=${borderCountries}`,
    ))

  const bordersArr =
    borderCountries &&
    borders.data.map(border => ({
      name: border.name,
